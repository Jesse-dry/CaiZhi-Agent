...
用法:
    ...
    python scripts/expand_feynman_dataset.py --chain-id C001 --count 3
"""
...
def main():
    parser = argparse.ArgumentParser(description="扩充费曼评价数据集")
    ...
    target = ExpansionTarget(
        ...
        output_types=["feynman"],
        target_count=args.count,
        generate_student_answers=False,
    )

    print(f"🎯 扩充费曼数据: {knowledge_ids} (chain={args.chain_id})")
    ...
    print(f"\n✅ 生成完成！")
    print(f"   费曼任务: {len(batch.feynman_tasks)}")
    print(f"   费曼回答: {len(batch.feynman_responses)}")
